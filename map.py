...
class Obstacle(pygame.sprite.Sprite):
    def __init__(self, pos,side,num=0) -> None:
        ...
        self.images = load_folder_images('obstacles')
...
class Map:
    def __init__(self):
        ...
        for _ in range(obtacles_per_row):
            ...
            x += self.top_obstacles.sprites()[-1].image.get_width()-2
        ...
        for _ in range(obstacles_per_column):
            self.left_obstacles.add(Obstacle((20,y+20),'left',randint(0,2)))
            y += offset_y
        
        # # columna derecha
        y= 0
        for _ in range(obstacles_per_column):
            self.right_obstacles.add(Obstacle((x+20,y+20),'right',randint(0,2)))
            y += offset_y
        ...
        x= 0
        for _ in range(obtacles_per_row):
            self.bottom_obstacles.add(Obstacle((20+x,y-7),'bottom',randint(0,2)))
            x += width-2
